...
def calculate_iv_rank_percentile(ticker):
    """Calculate IV Rank and IV Percentile over the past 52 weeks.
    ...
    try:
        ...
        hist['hv20'] = hist['returns'].rolling(20).std() * np.sqrt(252)
        hist = hist.dropna(subset=['hv20'])
        if hist.empty:
            return None, None
        current_iv   = hist['hv20'].iloc[-1]
        iv_52w_high  = hist['hv20'].max()
        iv_52w_low   = hist['hv20'].min()
        if (iv_52w_high - iv_52w_low) > 0:
            iv_rank = (current_iv - iv_52w_low) / (iv_52w_high - iv_52w_low) * 100
        else:
            iv_rank = 0.0
        iv_percentile = (hist['hv20'] < current_iv).mean() * 100
        return round(iv_rank, 1), round(iv_percentile, 1)
    except Exception as e:
        print(f"Error calculating IV Rank/Percentile: {e}")
        return None, None
...
def calculate_heston_price(flag, S, K, t, r, v0, kappa, theta, sigma_v, rho, q=0):
    ...
    try:
        ...
        P1 = 0.5 + (1.0 / np.pi) * quad(integrand, 1e-9, 200, args=(1,), limit=200)[0]
        P2 = 0.5 + (1.0 / np.pi) * quad(integrand, 1e-9, 200, args=(2,), limit=200)[0]
...
def calculate_heston_iv(price, S, K, t, r, flag, q=0):
    """Back-out implied volatility using Heston model calibration.
    Falls back to Black-Scholes Newton-Raphson if Heston solver fails.
    Default parameters are typical market starting values.
    """
    try:
        from scipy.optimize import brentq
        # Default Heston calibration starting parameters
        v0      = 0.04   # initial variance (~20% IV)
        kappa   = 2.0    # mean reversion speed
        theta   = 0.04   # long-run variance
        sigma_v = 0.3    # vol of vol
        rho     = -0.7   # typical negative asset/vol correlation

        def objective(sigma):
            hp = calculate_heston_price(flag, S, K, t, r, sigma**2, kappa, theta, sigma_v, rho, q)
            if hp is None:
                return 1e6
            return hp - price

        try:
            iv = brentq(objective, 0.001, 5.0, xtol=1e-5, maxiter=50)
            if 0.001 < iv < 5.0:
                return iv
        except Exception:
            pass
        # Fallback to Black-Scholes IV
        return calculate_implied_volatility(price, S, K, t, r, flag, q)
    except Exception:
        return calculate_implied_volatility(price, S, K, t, r, flag, q)
    ...
    with main_placeholder.container():
        ...
        if ticker:
            ...
            if len(recent_data) >= 2:
                ...
                iv_rank_val, iv_pct_val = calculate_iv_rank_percentile(ticker)
                if iv_rank_val is not None:
                    iv_cols = st.columns(2)
                    with iv_cols[0]:
                        rank_emoji = "\U0001f7e2" if iv_rank_val < 30 else ("\U0001f7e1" if iv_rank_val < 70 else "\U0001f534")
                        st.metric(
                            label=f"{rank_emoji} IV Rank (52w)",
                            value=f"{iv_rank_val:.1f}%",
                            help="0% = lowest IV in a year | 100% = highest IV in a year"
                        )
                    with iv_cols[1]:
                        pct_emoji = "\U0001f7e2" if iv_pct_val < 30 else ("\U0001f7e1" if iv_pct_val < 70 else "\U0001f534")
                        st.metric(
                            label=f"{pct_emoji} IV Percentile (52w)",
                            value=f"{iv_pct_val:.1f}%",
                            help="% of days in the past year where IV was below current level"
                        )
